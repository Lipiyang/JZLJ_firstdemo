...
export default {
	...
	methods:{
		run:function(CarName){			
			...
			// ./image/monitor_vehicle/car1.png
			...
			function resetMkPoint(map, sum, cun, obj_line,th,CarName,point_pre){
				var car_i = "{'id':" + cun[sum] + ", 'Carname':"+CarName+"}"
                  ...
                  token: th.$store.state.token
					...
					// var point_aft = new BMap.Point(ve_info_aft[0].CAR_LONG, ve_info_aft[0].CAR_LAT);
					...
					// const BMapLib = require('BMapLib')
					console.log(BMapLib.GeoUtils.isPointInPolygon(point_aft, th.apolygon))
					console.log(BMapLib.GeoUtils.isPointInPolygon(point_aft, th.apolygon_admin_region))
                    // map.centerAndZoom(point_aft, 16);
