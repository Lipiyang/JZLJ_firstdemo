<template>
    <div>     
            ...
            <Modal v-model="submodal" @on-ok="preserved" draggable scrollable title="信息录入表" width="1000" :transfer="false">
                ...
                <!-- 选址弹窗 -->
                <Modal v-model="mapmodal" width="600" :transfer="false">
                        <div id="r-result">请输入:<input type="text" id="suggestId" v-model="sourceAddress" size="20" style="width:150px;" /></div>
            ...
            preserved(){
                ...
                if(this.sourceAddress){
                     axios({
                        ...
                        .then(response => {
                            // console.log('测试：'+JSON.stringify(response.data.result.location))
                            this.sourceLong=response.data.result.location.lng
                            this.sourceLat=response.data.result.location.lat
                        axios({
                            headers: {
                            token: this.$store.state.token
                            },
