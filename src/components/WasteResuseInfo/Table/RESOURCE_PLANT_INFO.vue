<template>
  <div>
    ...
    <div
      ...
      title="基本信息"
      width="2500"
      style="width=2500px;margin-top:50px;margin-left:50px;"
    >
      <Row>
        <!-- 定义增加和删除按钮 -->
        <i-Col span="12">
    ...
    del() {
      for (var i = 0; i < this.initData.length; i++) {
        for (var j = 0; j < this.selection.length; j++) {
          if (
            this.initData[i]["resourcePlantId"] ==
            this.selection[j]["resourcePlantId"]
          ) {
            ...
            axios
              .put(
                "/api/resourcePlantInfo/" + this.selection[j]["resourcePlantId"],{headers: {
          token: this.$store.state.token
        }}
              )
              .catch(error => console.log(error));
