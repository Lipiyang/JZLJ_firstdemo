<template>
<div id="div45" v-cloak>
        //做查询的地方
        <div  v-if="showdiv45" class="demo div4 " >

                <h1>>>信息查询</h1>
            
                <div style=" width:580px;height: 40px; margin-top: 15px; margin-left:10px; magrin-right:10px;position: relative;">  
                    
                    <button class="button"  v-on:click="addInQuery"   style="font-size: 11px;">增加</button>
                    <button class="button"  v-on:click="remove"  style="font-size: 11px;background:red ">删除</button>
                     <button class="button"  v-on:click="updateInQuery"  style="font-size: 11px;background:yellow">更改</button>
                    <label>筛选</label>
                    <input type="text" v-model="query">
                    <input  class="button"  type="button" @click="showdiv45=false"  value="退出"  style="font-size: 11px;position: absolute;right: 0;" >  <br>
                </div> 
                <div style=" width:550px;height: 20px; margin-top: 5px;margin-bottom: 5px;margin-left:20px; magrin-right:10px;">
                <span >
                    <input  type="radio" value="all" v-model="radioValue">
                    <label>全选</label>
                </span>
                <span>
                    <input   type="radio" value="ops" v-model="radioValue">
                    <label>反选</label>
                </span>
                <!-- <span>
                    <input type="radio" value="no" v-model="radioValue">
                    <label>不选</label>
                </span> -->
                 </div>
                <!-- style="width:6.18px;height:10px;font-size: 5px;line-height: 10px;" -->
    ...
    computed: {
        ...
        filterconsumers:function(){
            ...
            return this.consumers.filter(function (item) {                         
                ...
                return item.name.indexOf(vm.query) !== -1
                })
        }
    },
    watch: {
...
.div5{
    margin: auto;
    position: absolute;
    background-color: hsl(133, 62%, 11%);;
    top: 0;
    bottom: 0;
    right: 0;
    left: 0;
